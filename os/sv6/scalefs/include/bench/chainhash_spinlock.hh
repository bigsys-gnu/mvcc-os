...
private:
  struct item /*: public rcu_freed*/ {
    ...
    : /*rcu_freed("::item", this, sizeof(*this)),*/
      key(k), val(v) {}
    void do_gc() { delete this; }
    NEW_DELETE_OPS(item);
...
public:
  ...
  bool replace_from(const K& kdst, const V* vpdst, chainhash_spinlock* src,
                    const K& ksrc, const V& vsrc, chainhash_spinlock *subdir,
  ...
  void enumerate(CB cb) const {
    // scoped_gc_epoch rcu_read;  // RCU read lock
